...
export function DashboardScreen() {
  ...
  return (
    <Container className="pb-14 pt-9">
      <PageHeader eyebrow="Index dashboard" title="Airfare Price Index — All India">
      ...
      <Async state={series} skeleton={<div className="mt-7"><SkeletonBlock height={340} label="Loading index series" /></div>} isEmpty={(a) => a.points.length === 0} empty={{ title: "No observations for this frequency", hint: "Try another frequency." }}>
        {(a) => {
          ...
          return (
            <>
              <RuleGrid className="mt-7">
                <StatTile label="Current level" value={v[v.length - 1].toFixed(1)} />
              ...
              <div className="border border-t-0 border-line px-5 pb-[22px] pt-[26px] md:px-7">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        <Async state={subs} skeleton={<SkeletonBlock height={220} label="Loading sub-indices" />}>
          {(s) => (
            <>
              <Card className="!px-[26px] !py-6">
                <CardTitle sub={`Weighted by passenger traffic share, ${s.weight_source}.`}>Sub-indices</CardTitle>
                <DataTable
                  caption="Sub-indices" minWidth={560} rowKey={(r) => r.code} rows={s.items}
                  columns={[
